...
struct cache_profile {
    ...
    constexpr cache_profile() noexcept
        : wr_hits(0u), rd_hits(0u)
        , wr_misses(0u), rd_misses(0u)
        , evictions(0u), write_backs(0u)
    {}
    
    constexpr cache_profile(const cache_profile &other) noexcept
        : wr_hits(other.wr_hits), rd_hits(other.rd_hits)
        , wr_misses(other.wr_misses), rd_misses(other.rd_misses)
        , evictions(other.evictions), write_backs(other.write_backs)
    {}

    constexpr cache_profile &operator=(const cache_profile &other) noexcept
    {
        if (this != &other) {
            wr_hits = other.wr_hits;
            rd_hits = other.rd_hits;
            wr_misses = other.wr_misses;
            rd_misses = other.rd_misses;
...
public:
    ...
    void insert(cache &c, ptrdiff_t loc, 
